	...
	for($i=0; $i < $total_categories; $i++) {
		...
		for ($x=0; $x < count($forum_row); $x++) {
			...
			if ($forum_row[$x]["cat_id"] == $categories[$i]["cat_id"]) {
				...
				$name = htmlspecialchars(stripslashes($forum_row[$x]["forum_name"]),ENT_QUOTES,'UTF-8');
				$last_post_nom = htmlspecialchars($forum_row[$x]["nom"],ENT_QUOTES,'UTF-8');
				$last_post_prenom = htmlspecialchars($forum_row[$x]["prenom"],ENT_QUOTES,'UTF-8');
				$last_post_topic_id = $forum_row[$x]["topic_id"];
				$total_posts = $forum_row[$x]["forum_posts"];
				$total_topics = $forum_row[$x]["forum_topics"];
				$desc = htmlspecialchars(stripslashes($forum_row[$x]["forum_desc"]),ENT_QUOTES,'UTF-8');
				$tool_content .= "<td>";
